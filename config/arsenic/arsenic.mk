#
#  Arsenic Optimizaciones
#
# TARGET_ARSENIC_AND can be set before this file to override the default of gcc 4.8 for ROM.
# This is to avoid hardcoding the gcc versions for the ROM and kernels.

 TARGET_ARSENIC_AND := $(TARGET_GCC_VERSION)
 TARGET_ARSENIC_KERNEL := $(TARGET_GCC_VERSION_KERNEL)
 ...
 TARGET_ARCH_LIB_PATH := $(ANDROID_BUILD_TOP)/prebuilts/gcc/$(HOST_PREBUILT_TAG)/arm/arm-linux-androideabi-$(TARGET_ARSENIC_AND)/lib
 export TARGET_ARCH_LIB_PATH := $(ANDROID_BUILD_TOP)/prebuilts/gcc/$(HOST_PREBUILT_TAG)/arm/arm-linux-androideabi-$(TARGET_ARSENIC_AND)/lib

 # Path to ROM toolchain
 ARSENIC_AND_PATH := $(ANDROID_BUILD_TOP)/prebuilts/gcc/linux-x86/arm/arm-linux-androideabi-$(TARGET_ARSENIC_AND)
...
ifeq (true,$(ENABLE_GOLD_LINKER))
  ...
  ifneq ($(GCC_OPTIMIZATION_LEVELS),)
    PRODUCT_PROPERTY_OVERRIDES += \
      ro.arsenic.flags=$(GCC_OPTIMIZATION_LEVELS)
  endif
  ifneq ($(GCC_OPTIMIZATION_LEVELS1),)
    PRODUCT_PROPERTY_OVERRIDES += \
      ro.arsenic.flags1=$(GCC_OPTIMIZATION_LEVELS1)
  endif
